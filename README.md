...
The interface is built by JavaScript, so everything a crawler or a link preview
needs lives in `index.html`: a descriptive title, a meta description, a
canonical URL, Open Graph and Twitter card tags pointing at
`icons/social-card.png` (generated by `npm run icons`), `WebApplication`
structured data, and a visually hidden summary paragraph under the page's only
`h1` so there is real text even without scripts. `test/seo.test.js` checks all
of it, including the card's dimensions.

Two things GitHub cannot serve from this repository: a `robots.txt` for the
whole domain (project sites only get `/<repo>/`, and crawlers read
`/robots.txt` at the root), and the repository description shown in search
results. `sitemap.xml` is here and can be submitted to Search Console as
